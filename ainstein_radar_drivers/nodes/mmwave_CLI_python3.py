...
def send_config(config_file, cmd_port, cmd_baud, silent, in_background):
    ...
    for i in config:
        write_str =  i+'\n'
        CLIport.write(write_str.encode())
...
def display_data(port, baud):
    ...
    # Check that the serial port is open
    if uart.isOpen() != True:
        print("Data Port didn't open, can't receive data from radar")
        return False
...
class PeopleCountingCLI:
    ...
    def readCSVToStruct(self, S, row_data, start_col):
        # row must be ordered as follows, from start_col: [range, azimuth, elevation, doppler, snr]
        # it may have more data after snr
        R = OrderedDict()
        q=start_col
        for key in S.keys():
            R[key] = float(row_data[q])
            q +=1
        return R
    ...
    def read_data(self,silent, TLVs_to_use):
        ...
        while (uart.isOpen() == True):
            while not lostSync:
                ...
                for i in range(8):
                    # print(rxHeader)
                    # print("Type: ", type(rxHeader[i]))
                    # print(rxHeader[i])
                    magicBytes.append(rxHeader[i])
                    # magicBytes.append(struct.unpack('<H',rxHeader[i])[0])

                if not np.array_equal(magicBytes, syncPattern):
                    print("array isn't equal, magicBytes = ",magicBytes,", syncPattern = ",syncPattern)
                ...
                if gotHeader:
                    if (frameHeader['frameNumber'] > targetFrameNum):
                        ...
                        print("Found sync at frame ",targetFrameNum,"(",frameNum,")")
                        gotHeader = False
                    else:
                        print("frameNumber = ",frameHeader['frameNumber'])
                        reason = "Old Frame"
                        gotHeader = False
                        lostSync  = True
                        break
                ...
                if not silent:
                    print("frameHeader['packetLength'] = ",frameHeader['packetLength'])
                ...
                if (dataLength > 0):
                    if not silent:
                        print("dataLength = ",dataLength,", frameHeaderLength = ",frameHeaderLengthInBytes)
                    # Read all packet
            ...
            if targetFrameNum:
                bytesAvailable = uart.inWaiting()
            print("Lost sync at frame ",targetFrameNum,"(",frameNum, "), Reason: ",reason,", ",bytesAvailable," bytes in Rx buffer")
            # print("Lost sync at frame ",targetFrameNum, "(",frameNum,"), Reason: ",reason, ", ",bytesAvailable," bytes in Rx buffer")
            ...
            while lostSync:
                ...
                while lostSync:
                    ...
                    if (count == 8):
                        ...
                        rxHeader=syncPatternUINT8.encode() # make an array of bytes
                        for i in range(frameHeaderLengthInBytes - 8):
                            rxHeader += bytes(header[i:i+1])
                        #     rxHeader.append(header[i])
