#ifndef RADAR_TARGET_CARTESIAN_ARRAY_VISUAL_H
#define RADAR_TARGET_CARTESIAN_ARRAY_VISUAL_H

#include <ainstein_radar_msgs/RadarTargetCartesianArray.h>
#include "radar_target_cartesian_visual.h"
...
class RadarTargetCartesianArrayVisual
{
public:
  RadarTargetCartesianArrayVisual( Ogre::SceneManager* scene_manager, Ogre::SceneNode* parent_node );
  virtual ~RadarTargetCartesianArrayVisual();

  void setMessage( const ainstein_radar_msgs::RadarTargetCartesianArray::ConstPtr& msg );
  void clearMessage( void );

  // Set the pose of the coordinate frame the message refers to.
  // These could be done inside setMessage(), but that would require
  // calls to FrameManager and error handling inside setMessage(),
  // which doesn't seem as clean.  This way RadarTargetCartesianArrayVisual is
...
private:
  ...
  std::vector<RadarTargetCartesianVisual> radar_target_cartesian_visuals_;
  ...
  // Nominal cartesian target shape scale (size):
  float scale_;
};
 
} // namespace ainstein_radar_rviz_plugins

#endif // RADAR_TARGET_CARTESIAN_ARRAY_VISUAL_H
